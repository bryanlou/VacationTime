...
const Nav = ({ authed }) => (
  <nav className="fixed flex items-center justify-between flex-wrap w-full py-4 px-6 bg-black">
	...
	C430.607,126.934,464.363,87.021,445.355,67.036z"
      ...
      <span className="text-xl font-semibold tracking-tight">Vacation Time</span>
    </div>
    <div>
      {authed && (
        <button
          onClick={() => EventBus.emit('logout')}
          className="inline-block px-4 py-3 mr-4 text-sm text-black hover:text-white focus:outline-none">
          Logout
        </button>
      )}
      <button
        onClick={() => EventBus.emit('addPlace')}
        className="inline-block px-4 py-3 border rounded text-white text-md leading-none border-white hover:border-transparent hover:text-black hover:bg-white focus:outline-none focus:bg-white focus:text-black">
        Add Place
      </button>
    </div>
  </nav>
